...
| `LLM_FALLBACK_MODELS` | Comma-separated ordered list of models to try when the primary `LLM_MODEL` fails. Applies to **all** LLM call paths — the summary worker's Map/Reduce (`Call`/`CallStream`), the agent chat client, the agent tools (merge/narrow/summarize) and API refine. Triggers on 429/5xx after retries **and on 401/403 account-level denials** (e.g. a Bedrock SCP explicit-deny), which escalate to the next model immediately (issue #211). Empty preserves single-model behavior. Example: `claude-sonnet-4-6,gpt-4.1`. Fallback only survives a provider/account outage when at least one fallback model is routed by `LLM_API_URL` to a **different backend/credential** than the primary (all models otherwise share the same gateway `LLM_API_KEY`). For the agent chat path, to leave budget for a fallback keep `maxAttempts*LLM_TIMEOUT + backoffs (3s) <= AGENT_STEP_TIMEOUT` — that path escalates early when the remaining deadline is too tight for another full-timeout attempt. | No | — |
