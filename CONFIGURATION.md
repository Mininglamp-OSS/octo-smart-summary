...
| `LLM_FALLBACK_MODELS` | Comma-separated ordered list of models to try when the primary `LLM_MODEL` fails. Applies to **all** LLM call paths — the summary worker's Map/Reduce (`Call`/`CallStream`), the agent chat client, the agent tools (merge/narrow/summarize) and API refine. Triggers on 429/5xx after retries and on **403 account-level denials** (e.g. a Bedrock SCP explicit-deny), which escalate to the next model immediately; 401 remains terminal because every model uses the same gateway credential. Empty preserves single-model behavior. Example: `claude-sonnet-4-6,gpt-4.1`. Fallback only survives a provider/account outage when at least one fallback model is routed by `LLM_API_URL` to a **different backend/credential** than the primary (all models otherwise share the same gateway `LLM_API_KEY`). Map/Reduce selects a model independently for each chunk, so mixed-model output is possible; cross-chunk stickiness is not currently provided. Worker/API paths have no aggregate fallback deadline: worst-case latency grows with the number of configured models, so keep the list short and size upstream/task deadlines accordingly. For the agent chat path, keep `maxAttempts*LLM_TIMEOUT + backoffs (3s) <= AGENT_STEP_TIMEOUT`; that path escalates early when the remaining deadline is too tight for another full-timeout attempt. | No | — |
