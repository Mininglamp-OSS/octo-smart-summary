...
func RecognizeIntent(
	...
	// Build member list for prompt (truncate to avoid prompt explosion)
	...
	if len(memberMap) > 0 {
		...
		if len(members) > maxMembers {
			log.Printf("[intent] WARN: member list truncated %d → %d", len(members), maxMembers)
	...
	// Build channel list for prompt (truncate to avoid prompt explosion)
	const maxChannels = 200
	var channelLines []string
	if len(channels) > 0 {
		channelsToProcess := channels
		if len(channelsToProcess) > maxChannels {
			log.Printf("[intent] WARN: channel list truncated %d → %d", len(channelsToProcess), maxChannels)
			channelsToProcess = channelsToProcess[:maxChannels]
		}
		for _, ch := range channelsToProcess {
			chType := "group"
...
func RecognizeIntentWithShortcut(
	...
	if ShouldSkipIntentRecognition(topic, specifiedSources, enableShortcut) {
		// Additional check: if topic contains any member name, don't shortcut
		// This handles cases like "总结张三的发言" where "张三" is not a @mention
		if topicContainsMemberName(topic, memberMap) {
			log.Printf("[intent] shortcut blocked: topic contains member name")
		} else {
...
// topicContainsMemberName checks if topic contains any member name from memberMap.
// Returns true if any member name (>= 2 runes) is found in the topic.
func topicContainsMemberName(topic string, memberMap map[string]string) bool {
	if len(memberMap) == 0 {
		return false
	}
	topicLower := strings.ToLower(topic)
	for _, name := range memberMap {
		// Skip empty or very short names (likely to cause false positives)
		// Use utf8.RuneCountInString for correct character count (not byte count)
		// e.g., "李" is 3 bytes but 1 rune
		if utf8.RuneCountInString(name) < 2 {
			continue
		}
		if strings.Contains(topicLower, strings.ToLower(name)) {
			return true
		}
	}
	return false
