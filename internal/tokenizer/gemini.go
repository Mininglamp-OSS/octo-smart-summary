...
func NewGeminiTokenizer(model string, cfg Config) *GeminiTokenizer {
	...
	// No default project ID - if not set, we'll fall back to estimate mode
	// to avoid leaking internal project IDs to open source repos
	...
	// If no project ID configured, return tokenizer that always falls back to estimate
	if projectID == "" {
		log.Printf("[tokenizer] GOOGLE_CLOUD_PROJECT not set, Gemini tokenizer will use estimate mode")
...
// IsExact returns false because Gemini uses API-based counting which can fail
