...
func loadKimiEncoding() (*tiktoken.Tiktoken, error) {
	// Try to get models directory from environment variable first
	modelsDir := os.Getenv("TOKENIZER_MODELS_DIR")
	if modelsDir != "" {
		modelsDir = filepath.Join(modelsDir, "kimi")
	} else {
		// Fallback: try relative to current working directory
		if _, err := os.Stat("internal/tokenizer/models/kimi"); err == nil {
			modelsDir = "internal/tokenizer/models/kimi"
		} else {
			// Last resort: try relative to executable
			if exe, err := os.Executable(); err == nil {
				dir := filepath.Join(filepath.Dir(exe), "models", "kimi")
	...
	if modelsDir == "" {
		return nil, fmt.Errorf("TOKENIZER_MODELS_DIR not set and models directory not found")
	}
