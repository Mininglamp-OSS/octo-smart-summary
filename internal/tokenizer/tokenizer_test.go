...
func TestKimiTokenizer_LocalTiktoken(t *testing.T) {
	...
	// Local tiktoken should be available
	if !tok.IsExact() {
		t.Error("KimiTokenizer should return IsExact()=true when local tiktoken is loaded")
	}
...
func TestClaudeTokenizer_WithEnvConfig(t *testing.T) {
	...
	// IsExact requires both API key and CLAUDE_COUNT_TOKENS_URL
	if tok.apiKey != "" && tok.countURL != "" {
		if !tok.IsExact() {
			t.Error("ClaudeTokenizer should return IsExact()=true when API key and URL are available")
		}

		// Test token counting - might fall back to estimate if API fails
		text := "Hello, world!"
		count := tok.Count(text)
		if count <= 0 {
			t.Errorf("ClaudeTokenizer.Count() should return positive value, got %d", count)
		}
		t.Logf("ClaudeTokenizer.Count() returned %d tokens for '%s'", count, text)
	} else {
		// Without both API key and URL, should fall back to estimate
		if tok.IsExact() {
			t.Log("ClaudeTokenizer returning IsExact()=false (missing API key or CLAUDE_COUNT_TOKENS_URL)")
		}
	}
}
...
func TestGeminiTokenizer_WithEnvConfig(t *testing.T) {
	...
	// IsExact requires both API key and OPENAI_API_BASE
	if tok.apiKey != "" && tok.baseURL != "" {
		if !tok.IsExact() {
			t.Error("GeminiTokenizer should return IsExact()=true when API key and base URL are available")
		}
		...
		t.Logf("GeminiTokenizer.Count() returned %d tokens for '%s'", count, text)
	} else {
		// Without both API key and base URL, should fall back to estimate
		if tok.IsExact() {
			t.Log("GeminiTokenizer returning IsExact()=false (missing OPENAI_API_KEY or OPENAI_API_BASE)")
		}
	}
}
