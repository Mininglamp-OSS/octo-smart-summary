...
CREATE TABLE `agent_summary_run` (
    ...
    `coverage_measured`  TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'whether channel fetch coverage was actually observed',
    `attempted_channels` JSON        NOT NULL COMMENT 'unique channel ids passed to fetch_channel',
    `succeeded_channels` JSON        NOT NULL COMMENT 'attempted channels whose latest fetch succeeded, including quiet channels',
    `failed_channels`    JSON        NOT NULL COMMENT 'attempted channels whose latest fetch failed',
    `coverage_truncated` TINYINT(1)  NOT NULL DEFAULT 0 COMMENT 'at least one successful fetch hit its message cap',
    `dropped_messages`   INT         NOT NULL DEFAULT 0 COMMENT 'messages discarded before summarization, including frozen-manifest misses',
